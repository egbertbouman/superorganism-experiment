...
class CreateSolutionOverlay(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        self.title_field = TextFieldBlock(
            ...
            placeholder_text="e.g., Add staged review before voting",
            max_length=SOLUTION_TITLE_MAX_LENGTH,
            parent=self.card,
        )
        ...
        self.description_field = TextAreaFieldBlock(
            ...
            max_length=SOLUTION_DESCRIPTION_MAX_LENGTH,
            parent=self.card,
        )
        self.description_field.textChanged.connect(self._on_description_changed)

        self.payout_address_field = TextFieldBlock(
            title="Bitcoin Payout Address",
            placeholder_text="bc1...",
            parent=self.card,
        )
        self.payout_address_field.textChanged.connect(self._on_funding_fields_changed)

        self.asking_price_field = TextFieldBlock(
            title="Asking Price (satoshis)",
            placeholder_text="e.g., 250000",
            parent=self.card,
        )
        self.asking_price_field.textChanged.connect(self._on_funding_fields_changed)

        self.deadline_height_offset_field = TextFieldBlock(
            title="Deadline Height Offset",
            placeholder_text="e.g., 144",
            parent=self.card,
        )
        self.deadline_height_offset_field.textChanged.connect(
            self._on_funding_fields_changed
        )

        self.action_block = DialogActionBlock(
            primary_text="Create Solution",
        ...
        card_layout.addWidget(self.payout_address_field)
        card_layout.addWidget(self.asking_price_field)
        card_layout.addWidget(self.deadline_height_offset_field)
    ...
    def _clear_fields(self) -> None:
        ...
        self.payout_address_field.clear()
        self.asking_price_field.clear()
        self.deadline_height_offset_field.clear()
        self._clear_errors()
    ...
    def _clear_errors(self) -> None:
        ...
        self.payout_address_field.clear_error()
        self.asking_price_field.clear_error()
        self.deadline_height_offset_field.clear_error()
    ...
    def _current_draft(self) -> SolutionDraft:
        return SolutionDraft(
            ...
            bitcoin_payout_address=self.payout_address_field.text(),
            asking_price_satoshis=self.asking_price_field.text(),
            deadline_height_offset=self.deadline_height_offset_field.text(),
        )
    ...
    def _validate_form(self) -> bool:
        ...
        self._apply_funding_errors(draft.validate_funding_fields())
    ...
    def _apply_funding_errors(self, errors: dict[str, str]) -> None:
        payout_address_error = errors.get("bitcoin_payout_address", "")
        asking_price_error = errors.get("asking_price_satoshis", "")
        deadline_height_offset_error = errors.get("deadline_height_offset", "")

        self._set_error("bitcoin_payout_address", payout_address_error)
        self._set_error("asking_price_satoshis", asking_price_error)
        self._set_error("deadline_height_offset", deadline_height_offset_error)

        self.payout_address_field.set_error(payout_address_error)
        self.asking_price_field.set_error(asking_price_error)
        self.deadline_height_offset_field.set_error(deadline_height_offset_error)
    ...
    def _on_funding_fields_changed(self, _text: str) -> None:
        draft = self._current_draft()
        self._apply_funding_errors(draft.validate_funding_fields())
        self._update_create_button_state()
